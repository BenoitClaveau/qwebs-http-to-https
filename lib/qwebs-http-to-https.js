...
class HttpServer {
  constructor($config, $qwebs) {
		...
		if (!$config.host) throw new DataError({ message: "Https host is not defined in qwebs config."});
		
		this.$config = $config;
		this.$qwebs = $qwebs;
			
		http.createServer((request, response) => {
				return response.redirect({ url: this.$config.host + request.url }).catch(error => {
